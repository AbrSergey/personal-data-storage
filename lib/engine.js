...
const { AddWord, AddFile, Search, Delete } = require("./wordIndex");
...
const updateWIndex = async (data, id, opts) => {
  ...
  Object.keys(data).forEach(async key => {
    tmp.push(new Promise( async resolve => {
      if (_search[key] === "word"){
        let promises = [];
        uniq(data[key].split(" ")).forEach(word => promises.push(AddWord(word, id, _word_index_id[key], _indexAModel, _indexTModel, opts)));
        const arrAsId = await Promise.all(promises);
  
        let objAsId;
        arrAsId.map(obj => objAsId = {...objAsId, ...obj});
  
        await AddFile(id, objAsId, _word_index_id[key], _indexAModel, _indexTModel, opts);
        resolve();
      } else resolve();
    }))
  });
...
const deleteWIndex = async (fileId) => {
  ...
  Object.keys(_search).forEach(async key => {
    promises.push(new Promise(async resolve => {
      if (_search[key] === "word"){
        //??????????????
        await Delete(fileId, _word_index_id[key], _indexAModel, _indexTModel);
        resolve();
      } else resolve();
    }))
  });
...
module.exports.setSearchSettings = async (schema, opts) => {
...
module.exports.find = async (key, value) => {
  if (_search[key] === undefined) {
    ...
    const arrId = await Search(value, _word_index_id[key], _indexAModel, _indexTModel);
    return arrId;
  }
};
