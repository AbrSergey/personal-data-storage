...
const updateWIndex = async (data, id, opts) => {
  Object.keys(data).forEach(async key => {
    if (_search[key] === "word"){
      let promises = [];
      data[key].split(" ").forEach(word => promises.push(AddWord(word, id, _word_index_id[key], _indexAModel, _indexTModel, opts)));
...
module.exports.deleteById = async (id) => {
  ...
  if (!object) return {};
  await checkSignature(object);
  await Delete(id, _word_index_id, _indexAModel, _indexTModel)
