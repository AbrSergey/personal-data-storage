...
const { setWIndexSettings, AddWord, AddFile, Search, Delete } = require("./wordIndex");
...
const updateWIndex = async (data, id, opts) => {
  ...
  Object.keys(data).forEach(async key => {
    tmp.push(new Promise( async resolve => {
      if (_search[key] === "word"){
        let promises = [];
        unique(data[key].split(" ")).forEach(word => promises.push(AddWord(word, id, key, opts)));
        const arrAsId = await Promise.all(promises);
  
        let objAsId;
        arrAsId.map(obj => objAsId = {...objAsId, ...obj});
  
        await AddFile(id, objAsId, key, opts);
        resolve();
      } else resolve();
    }))
  });
...
module.exports.setIndexSettings = async (schema, opts) => {
...
module.exports.find = async (key, value) => {
  if (_search[key] === undefined) {
    ...
    const arrId = await Search(value, key);
    return arrId;
  }
};
