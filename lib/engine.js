...
class Engine {
  ...
  #open_keys; //array of keys
  #secure_keys; //array of keys
  #model;
  #search = {}; // undefined || "entire" || "word" || "nosearch"
  ...
  _encryptSecureFields(keys, data, initIv){
    ...
    keys.forEach((key, index) => {
      const iv = parseInt((initIv).toString().slice(0, 16), 16) % 1000000000 + this.#secure_keys.indexOf(key);
  ...
  _decryptSecureFields(data){
    ...
    this.#secure_keys.forEach((key, index) => {
      const iv = parseInt((data._id).toString().slice(0, 16), 16) % 1000000000 + index;
  ...
  async update(id, data, opts = {}){
    if (id && data){
      ...
      //add eindex
      await this._createEIndex(resObject, updatedSecureData);
      await this._deleteWIndex(id);
      await this._updateWIndex(updatedSecureData, id, opts);
  ...
  async find(key, value){
    ...
    if (this.#search[key] === undefined) {
      ...
      const arr_id = await this.#wIndex.Search(value, key);
      
      let result = [];
      if (Array.isArray(arr_id)){
        await Promise.all(arr_id.map(async id => {
          const [ object ] = await this.#model.find({ _id: id }).exec().catch(err => {
            throw new PersonalDataStorageError(err);
          });
