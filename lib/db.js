...
const { update, insert, getDataByID, findOneByKeyInNonSecureData, deleteById, setKey, generateModel, getModel, generateValidationScheme } = require("./engine");
const { connect, disconnect, getConnect } = require("./connect");
const { deleteUndefinedKey, checkSchemeOnValidate } = require("./helpers");
...
module.exports = {
  connect: async (url, schema, modelName, key, dbOptions, schemaOptions, algorithm) => {
    ...
    setKey(key);
    await checkSchemeOnValidate(schema);
    generateValidationScheme(schema);
