      ...
      if (getConnect()){
        ...
        if (verifySignature(objVerify, object.signature, _key)){
          ...
          if (secureKeys.length){
            securePart = decryptData(object.security, object.iv, _algorithm, _key);
            secureKeys.forEach(key => securePart[key] = data[key]);
            [ securePart, openPart.iv ] = encryptData(securePart, _algorithm, _key, _ivLen);
          }
          ...
          return _model.updateOne({ _id: openPart._id }, resObject, {upsert: true}); //promise
        }
