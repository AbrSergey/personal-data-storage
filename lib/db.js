  ...
  insert: (data) => {
    if (getConnect()){
      if (data && data.security){
        ...
        [ object.security, object.iv ] = encryptData(object.security, _algorithm, _password, _salt, _keyLen, _ivLen);
        object.signature = createSignature(object, _password);
  ...
  update: (id, data) => {
    if (id && data && data.security){
      if (getConnect()){
        ...
        if (verifySignature(objVerify, object.signature, _password)){
          // change data
        }
