...
class WIndex {
  ...
  async AddAs(word, file_id, key, opts){
    ...
    //check if this word exists in Ts
    ...
    let As;
    //if the word doesn’t exist then add an entry to the indexAs collection
    if (!Ts){
  ...
  async Delete(fileId, key){
    ...
    //calculate hash from id file
    const Fk1_file = this.#encryptor.F(fileId, this.#key1);
    const record_in_Td = `d.${Fk1_file}`;
  
    const Td = await this.#indexTModel.findOneAndUpdate(
      { _id: this.#word_index_id[key] },
    ...
    while (Ad_addr !== this.#null){
      ...
      //update Nd_plus and N+1 in index Ad at address Nd_minus on Nd_plus and N+1
      ...
      //updae Nd_minus and N-1 in index Ad at address Nd_plus on Nd_minus and N-1
      ...
      //update next in As at address N_1 to N + 1
      ...
      //if this is the first element, that is, N_minus is zero, and N_plus is not zero, then fix the pointer in Ts to N_plus
      ...
      //delete record in As at address N
      promises.push(this.#indexAModel.deleteOne({ _id: Ad.addrN }));
      await Promise.all(promises);
  
      //go to the next word in the file to be deleted
